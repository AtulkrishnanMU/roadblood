...
func _ready() -> void:
	...
	var random_scale = RandomCacheScript.get_scale(cache_index) * 1.28 + 0.72  # Adjust to 0.72-2.0 range (bigger)
...
func _on_collision(_body: Node) -> void:
	...
	if _body is TileMap or _body.is_in_group("ground") or _body.is_in_group("colliders"):
...
func _create_blood_decal() -> void:
	...
	if decal:
		...
		var cache_index = RandomCacheScript.get_random_index()
		decal.rotation = RandomCacheScript.get_rotation(cache_index) * 0.5  # Adjust to 0-PI range
		var scale_factor = RandomCacheScript.get_scale(cache_index) * 0.5 + 0.1  # Adjust to 0.2-0.6 range
